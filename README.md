...
<img width="508" alt="Снимок экрана 2021-12-12 в 17 07 57" src="https://user-images.githubusercontent.com/64102363/145715792-f89b1be7-483e-459f-836a-fa3d7a985fe9.png">
...
В среднем, хорошо проиндексированная база может держать 2500 rps [14], что с учетом нашей нагрузки в 310 000 rps, это приблизительно 124 сервера. 
Под хранение нужно 3 300 ТБ места в год, для быстрого доступа к данным возьмем ssd диски на 960 gb [14], что соответствует 3300 ssd на 124 сервера, получается не очень оптимально, поэтому старые сообщения, мы будем хранить на hdd, поскольку к ним не так часто обращаються, стандартный размер hdd в сервере это салазка на 32 дисков по 8db каждый, итого по 256gb  на сервер или 31 744TB на наши 124 сервера, что хватит нам на несколько лет вперед. Самые свежие сообщения будем хранить на 2х ssd дисках в сервере по 960gb каждый и постеменно переносить их на HDD
...
Итого, учитывая репликацию нам потребуется 248 серверов с базами данных
...
15. https://habr.com/ru/company/avito/blog/525294/
