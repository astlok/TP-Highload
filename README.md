...
### Про доставку сообщений по миру
Отдельно хотелось бы поговорить про доставку сообщений по миру, тк пользователи у нас со всего мира и присутствуют груповые чаты
Дабы снизить нагрузку на системы сделаем ограничение на количество пользователей в одной беседе до 50 человек (там долгое время было сделано в ВК)

На каждый из дата центров стоит поставить отдельный сервер/сервера, который будет знать, где располагаются другие дата центры и наши основные сервисы могли передать запрос на сервис, находящийся в другом ДЦ (новые сообщения, новые люди в чате) и таким образом все пользователи будут получать свои сообщения. Из-за ограничения в 50 человек в беседе это не создаст большой слишком большой нагрузки, а лишь немного уменьшит время доставки, что на этапе MVP вполне допустимо
